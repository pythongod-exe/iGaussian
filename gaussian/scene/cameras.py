...
# start_pose_w2c 初始相机姿态，从世界坐标系到相机坐标系的变换矩阵 FoVx 和 FoVy：相机的水平和垂直视场角
# image_width 和 image_height：图像的宽度和高度 trans：平移向量，默认值为 [0.0, 0.0, 0.0] scale：缩放因子，默认值为 1.0
class Camera_Pose(nn.Module):
    def __init__(self,start_pose_w2c, FoVx, FoVy, image_width, image_height,
        ...
        self.zfar = 100.0 # 远平面和近平面的深度，用于相机投影矩阵
        self.znear = 0.01
    ...
    def forward(self, start_pose_w2c):
        ...
        self.pose_w2c = torch.matmul(deltaT, start_pose_w2c.inverse()).inverse() # 将变换矩阵用于初始相机姿态上[4, 4]
        # self.pose_w2c = start_pose_w2c
        self.update()
    
    def current_campose_c2w(self):
        return self.pose_w2c.inverse().clone().cpu().detach().numpy() # 返回从相机坐标系到世界坐标系的变换矩阵
